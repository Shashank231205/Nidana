...
from packages.schemas.predicate import Predicate
from packages.schemas.registry import ComplaintFamily, FamilyRegistry, FieldType
from services.clinical.predicates import DEMOGRAPHIC_FIELDS
from services.clinical.registry_loader import RULES_ROOT, known_fields

PREDICATES_DIR: Final[Path] = RULES_ROOT / "predicates"
...
def load_predicates(predicates_dir: Path | None = None) -> dict[str, Predicate]:
    """Load every predicate, rejecting duplicate ids across files."""
    directory = predicates_dir if predicates_dir is not None else PREDICATES_DIR
